                    ...
                    results.append(
                        {
                            ...
                            "reasoning": data.get("reasoning_chain", [{}])[0].get(
                                "logic", "N/A"
                            ),
                        }
                    )
...
def generate_report(results):
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        for r in results:
            if r["status"] == "SUCCESS":
                f.write(
                    f"| {r['case']} | ✅ {r['status']} | {r['destination']} | {r['match_score']} | {r['reasoning'][:50]}... | {r['duration']} |\n"
                )
            else:
                f.write(
                    f"| {r['case']} | ❌ {r['status']} | N/A | N/A | {str(r.get('error', 'N/A'))[:50]}... | {r['duration']} |\n"
                )
